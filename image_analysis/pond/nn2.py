...
def quant_weights_tensor2(weights):
    ...
    quant_scheme = MinMaxObserver(dtype=torch.quint8, qscheme=torch.per_tensor_symmetric, reduce_range=False)
...
def quantize_tensor(x, num_bits=8):
    qmin = 0.
    qmax = 2.**num_bits - 1.
    # weights = x.unwrap()  # .astype(np.float32)
...
def quant_weights_filters2(weights):
    ...
    for i in range(weights.shape[0]):
        for j in range(weights.shape[1]):
            quanted_weights[i, j, :, :], scale, zero_point = quantize_tensor(weights[i, j, :, :])
